...
class Calculator {
	...
	def dispatch private internalEvaluate(Rel rel) {
		...
		if(evalLeft instanceof Boolean && evalRight instanceof Boolean){
			...
			val castRight = evalRight as Boolean
			switch (rel.relation){
				case GREATER: throw new MismatchingTypesException("Can only compare boolean values for (un)equality.")
				case GREATER_OR_EQUAL: throw new MismatchingTypesException("Can only compare boolean values for (un)equality.")
				case EQUAL: return castLeft == castRight
				case UNEQUAL: return castLeft != castRight
				case LESS_OR_EQUAL: throw new MismatchingTypesException("Can only compare boolean values for (un)equality.")
				case LESS: throw new MismatchingTypesException("Can only compare boolean values for (un)equality.")
			}
		}else if(evalLeft instanceof Double && evalRight instanceof Double){		
			// -------------------- Numerical Values -----------------------	
			val castLeft = evalLeft as Double
